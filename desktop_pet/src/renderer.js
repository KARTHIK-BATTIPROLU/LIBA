...
const FALLBACK_CHARACTER = {
  id: "liebe",
  name: "Liebe",
  spritePath: "../assets/characters/liebe/sprite.png",
  frame: { width: 768, height: 832 },
  states: {
    idle:         { row: 0, frames: 8, interval: 167 },
    runningRight: { row: 1, frames: 8, interval: 83  },
    runningLeft:  { row: 2, frames: 8, interval: 83  },
    waving:       { row: 3, frames: 8, interval: 125 },
    jumping:      { row: 4, frames: 8, interval: 100 },
    failed:       { row: 5, frames: 8, interval: 167 },
    running:      { row: 6, frames: 8, interval: 83  },
    review:       { row: 7, frames: 8, interval: 200 }
  },
  automaticActions: ["review", "waving", "jumping", "review"],
  clickActions: ["waving", "jumping", "failed", "review"],
  staticExpressions: [
    { state: "idle", frame: 0 },
    { state: "idle", frame: 2 },
    { state: "idle", frame: 4 },
    { state: "waving", frame: 3 },
    { state: "review", frame: 2 },
    { state: "failed", frame: 2 }
  ],
};
...
// ---------------------------------------------------------------------------
// LIBA Event Bridge — map orchestrator events to Liebe animations
// ---------------------------------------------------------------------------
const LIBA_STATE_MAP = {
  listening:  "waving",   // row 3 — Liebe perks up, notices user
  thinking:   "review",   // row 7 — Liebe thinking/reading
  speaking:   "idle",     // row 0 — gentle sway while Piper TTS plays
  executing:  "jumping",  // row 4 — Liebe leaps/slashes doing OS task
  idle:       "idle",     // row 0 — default
  error:      "failed",   // row 5 — slumped/frustrated
  sleeping:   "idle",     // row 0 — but at 1fps via override below
};

let libaOverrideToken = 0; // Used to cancel pending overrides when new event arrives

function handleLibaStateChange(eventName) {
  const nextState = LIBA_STATE_MAP[eventName];
  if (!nextState) return;

  libaOverrideToken += 1;
  const token = libaOverrideToken;

  if (eventName === "sleeping") {
    // Near-still: switch to idle but throttle animation to ~1 fps
    window.clearTimeout(idleTimer);
    window.clearInterval(animationTimer);
    state = "idle";
    frameIndex = 0;
    const current = character.states.idle || character.states[Object.keys(character.states)[0]];
    animationTimer = window.setInterval(() => {
      if (libaOverrideToken !== token) return; // Another event arrived — stop
      frameIndex = (frameIndex + 1) % (current ? current.frames : 8);
      draw();
    }, 1000); // 1 fps
    draw();
  } else if (["listening", "thinking", "executing", "error"].includes(eventName)) {
    // Temporary override — hold state until idle event returns us to normal
    window.clearTimeout(idleTimer);
    temporaryToken += 1;
    const tmpToken = temporaryToken;
    if (character.states[nextState]) {
      state = nextState;
      frameIndex = 0;
      startAnimation();
    }
    // Safety fallback: return to idle after 30s maximum even if idle event is missed
    window.setTimeout(() => {
      if (temporaryToken === tmpToken && libaOverrideToken === token) {
        setState("idle");
        scheduleIdleBehavior();
      }
    }, 30000);
  } else {
    // idle / speaking — return to normal
    if (character.states.idle) {
      setState("idle");
      scheduleIdleBehavior();
    }
  }
...
if (window.desktopPet.onLibaStateChange) {
  window.desktopPet.onLibaStateChange(handleLibaStateChange);
}
