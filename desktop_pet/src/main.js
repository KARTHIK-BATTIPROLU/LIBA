...
process.on("uncaughtException", (err) => logDebug("UNCAUGHT EXCEPTION:", err.stack || err));
process.on("unhandledRejection", (err) => logDebug("UNHANDLED REJECTION:", err));
logDebug("Electron main.js initializing...");

const i18n = require("./i18n");

// ---------------------------------------------------------------------------
// LIBA Event Bridge — WebSocket client connecting to the Python orchestrator.
// Forwards state events to the renderer as "liba-state-change" IPC messages.
// ---------------------------------------------------------------------------
const LIBA_BRIDGE_URL = "ws://localhost:8766";
const LIBA_RECONNECT_MS = 3000;
let libaWs = null;
let libaReconnectTimer = null;

function connectLibaBridge() {
  // Don't reconnect if we're already connected or connecting
  if (libaWs && (libaWs.readyState === 0 || libaWs.readyState === 1)) return;

  try {
    libaWs = new WebSocket(LIBA_BRIDGE_URL);

    libaWs.addEventListener("open", () => {
      console.log("[LIBA Bridge] Connected to", LIBA_BRIDGE_URL);
      if (libaReconnectTimer) { clearTimeout(libaReconnectTimer); libaReconnectTimer = null; }
    });

    libaWs.addEventListener("message", (event) => {
      try {
        const data = JSON.parse(event.data);
        const eventName = data && typeof data.event === "string" ? data.event : null;
        if (!eventName) return;
        console.log("[LIBA Bridge] Event:", eventName);
        // Forward to all renderers via IPC
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send("liba-state-change", eventName);
        }
      } catch {
    ...
    libaWs.addEventListener("close", () => {
      console.log("[LIBA Bridge] Disconnected — retrying in", LIBA_RECONNECT_MS, "ms");
      libaWs = null;
      scheduleLibaReconnect();
    });

    libaWs.addEventListener("error", () => {
      // Error fires before close; close handler schedules the retry
    });
  } catch (err) {
    console.warn("[LIBA Bridge] Could not create WebSocket:", err.message);
    scheduleLibaReconnect();
  }
}

function scheduleLibaReconnect() {
  if (libaReconnectTimer) return; // Already scheduled
  libaReconnectTimer = setTimeout(() => {
    libaReconnectTimer = null;
    connectLibaBridge();
  }, LIBA_RECONNECT_MS);
}
...
const DEFAULT_CHARACTER_ID = "liebe";
...
function createWindow() {
  ...
  mainWindow.webContents.on("did-fail-load", (_e, code, desc, url) => {
    logDebug("mainWindow did-fail-load:", code, desc, url);
  });
  mainWindow.webContents.on("console-message", (_e, level, message, line, sourceId) => {
    logDebug("mainWindow console:", level, message, line, sourceId);
  });
...
function createChatWindow() {
  ...
  logDebug("Creating chatWindow...");
  chatWindow = new BrowserWindow({
    width: 440,
    height: 580,
    minWidth: 360,
    minHeight: 420,
    x: Math.max(primary.x + 40, Math.round(primary.x + primary.width / 2 - 220)),
    y: Math.max(primary.y + 40, Math.round(primary.y + primary.height / 2 - 290)),
  ...
  chatWindow.webContents.on("did-fail-load", (_e, code, desc, url) => {
    logDebug("chatWindow did-fail-load:", code, desc, url);
  });
  chatWindow.webContents.on("console-message", (_e, level, message, line, sourceId) => {
    logDebug("chatWindow console:", level, message, line, sourceId);
  });
  ...
  logDebug("chatWindow created in screen center and loadFile called.");
}
...
function getGroqKeyPool(currentApiKey) {
  const keys = [];
  if (currentApiKey) keys.push(currentApiKey);
  try {
    const envPaths = [
    ...
    for (const p of envPaths) {
      if (fs.existsSync(p)) {
        ...
        const matches = raw.matchAll(/^GROQ_API_KEY(?:_\d+)?\s*=\s*(.+)$/gm);
...
async function requestAssistantReply(userText) {
  ...
  const keyPool = getGroqKeyPool(assistant.apiKey);
  const keysToTry = keyPool.length > 0 ? keyPool : [assistant.apiKey];
  ...
  for (let i = 0; i < keysToTry.length; i++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        lastDetail = await responseErrorText(response);
        // If rate limit (429) or auth error (401/403), try next key in pool
        if ([401, 403, 429].includes(response.status) && i < keysToTry.length - 1) {
          console.warn(`[Groq Fallback] Key #${i + 1} failed with status ${response.status}. Trying key #${i + 2}...`);
          continue;
        }
        return {
          reply: t("assistant.statusError", {
            status: response.status,
            detail: lastDetail ? `: ${lastDetail}` : "",
          }),
      ...
      if (i < keysToTry.length - 1) {
        console.warn(`[Groq Fallback] Request with key #${i + 1} errored (${error?.message}). Trying key #${i + 2}...`);
        continue;
      }
...
function createTray() {
  try {
    ...
    tray = new Tray(icon);
    tray.setToolTip("LIBA Desktop Pet (Liebe)");
    const contextMenu = Menu.buildFromTemplate([
      { label: "LIBA / Liebe Desktop Pet", enabled: false },
      { type: "separator" },
      { label: "💬 Open Chat Window (Ctrl+Shift+C)", click: createChatWindow },
      { label: "⚡ Quick Input (Ctrl+Shift+Space)", click: createQuickChatWindow },
      { label: "⚙️ Settings", click: createCompanionSettingsWindow },
      { type: "separator" },
      {
        label: "📍 Bring Pet to Center",
        click: () => {
          if (mainWindow && !mainWindow.isDestroyed()) {
            const primary = screen.getPrimaryDisplay().workArea;
            const size = displaySize();
            mainWindow.setPosition(
              Math.round(primary.x + primary.width / 2 - size.width / 2),
              Math.round(primary.y + primary.height / 2 - size.height / 2),
              true
            );
            mainWindow.show();
            mainWindow.setAlwaysOnTop(true, "screen-saver");
...
app.whenReady().then(() => {
  ...
  createWindow();
  logDebug("createWindow() called. Calling connectLibaBridge()...");
  connectLibaBridge(); // LIBA event bridge — start trying to connect immediately
  ...
  try {
    ...
    globalShortcut.register("CommandOrControl+Shift+Space", createQuickChatWindow);
    globalShortcut.register("CommandOrControl+Alt+L", createChatWindow);
  ...
  try {
    ...
    // Window icons are platform-dependent
  }
